<template>
    <div class="u-calendar">
        <div class="u-calendar__action u-flex u-row-center">
            ...
            <div class="u-calendar__action__icon_left">
                <div class="u-calendar__action__icon">
                    <div v-if="changeYear" class="iconfont icon-arrow-double-left" @click="changeYearHandler(0)"></div>
                </div>
                <div class="u-calendar__action__icon">
                    <div v-if="changeMonth" class="iconfont icon-arrow-left" @click="changeMonthHandler(0)"></div>
            ...
            <div class="u-calendar__action__icon_right">
                <div class="u-calendar__action__icon">
                    <div v-if="changeMonth" class="iconfont icon-arrow-right" @click="changeMonthHandler(1)"></div>
                    <!-- <u-icon v-if="changeMonth" name="arrow-right" :color="monthArrowColor" @click="changeMonthHandler(1)">
						</u-icon> -->
                </div>
                <div class="u-calendar__action__icon">
                    <div v-if="changeYear" class="iconfont icon-arrow-double-right" @click="changeYearHandler(1)"></div>
        ...
        <div class="u-calendar__content" v-if="mode == 'date'">
            ...
            }" v-for="(item, index) in daysArr" :key="index" @click="dateClick(index)">
        ...
        <div class="u-calendar__content" v-else-if="mode == 'rangeweek'">
            <div class="u-calendar__content_weeks_list">
                ...
                }" :key="'_week' + index" @click="getWeekDate(item)">
                    <div class="">
                        第{{ index + 1 }}周
                    </div>
        ...
        <div class="u-calendar__content" v-else>
            <div class="u-calendar__content_month_list">
                ...
                }" :key="'_month' + index" @click="getMonthDate(item.value)">
                    {{ item.label }}
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        init () {
            ...
            this.nowDay = `${now.getFullYear()}-${now.getMonth() + 1 < 10 ? '0' + (now.getMonth() + 1) : now.getMonth() + 1}-${now.getDate() < 10 ? '0' + now.getDate() : now.getDate()}`
            let nowMonth = ''
            ...
            this.weeksSelectAll.map((item, index) => {
                if (item.monday <= this.nowDay && item.sunday <= this.nowDay) {
        ...
        gettime () {
            ...
            weeksall.map(item => {
                if (item < this.currentMonthDays) {
                    let obj = {}
                    if (item <= 9 && item + 6 < 10) {
                        obj.monday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-0' + item
                        obj.sunday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-0' + (item + 6)
                    } else if (item <= 9 && item + 6 >= 10) {
                        obj.monday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-0' + item
                        obj.sunday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-' + (item + 6)
                    } else if (item > 9 && item + 6 < this.currentMonthDays) {
                        obj.monday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-' + item
                        obj.sunday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-' + (item + 6)
                    } else {
                        obj.monday = this.year + '-' + (this.month < 10 ? '0' + this.month : this.month) + '-' + item
                        obj.sunday = this.year + '-' + (this.month + 1 < 10 ? '0' + (this.month + 1) : this.month + 1) + '-' + '0' + (item + 6 - this.currentMonthDays)
                    }
                    obj.disabled = this.replaceDate(obj.sunday,this.nowDay)
        ...
        replaceDate (sunday, today) {
            if (!sunday) {
                return true
            }
            let newSunday = Number(sunday.replaceAll('-', ''))
            let newDay = Number(today.replaceAll('-', ''))
            console.log(newSunday, newDay)
            if (newSunday > newDay) {
                return true
            }
            return false
